...
def f_mi(x):
    ...
    d = []
    d.append((np.nanpercentile(x['norm_h'][x['classification'] > 1],percentile_intervals))) #canopy_rh
    d.append((np.nanpercentile(x['h_ph'][x['classification'] > 1],percentile_intervals))) #canopy_rh_abs
    d.append((x['norm_h'][x['classification'] > 1]).std()) #canopy_openness
    d.append((x['h_ph'][x['classification'] > 0]).mean()) # centroid_height
    d.append((np.sqrt(((x['norm_h'][x['classification'] > 1])/(x['norm_h'][x['classification'] > 1]).mean()).sum()))) # h_canopy_quad
    d.append((x['norm_h'][x['classification'] > 1]).max()) #h_max_canopy
    d.append((x['h_ph'][x['classification'] > 1]).max()) #h_max_canopy_abs
    d.append((x['norm_h'][x['classification'] > 1]).mean()) #h_mean_canopy
    d.append((x['h_ph'][x['classification'] > 1]).mean()) #h_mean_canopy_abs    
    d.append((x['norm_h'][x['classification'] > 1]).median()) #h_median_canopy
    d.append((x['h_ph'][x['classification'] > 1]).median()) #h_median_canopy_abs      
    d.append((x['norm_h'][x['classification'] > 1]).min()) #h_min_canopy
    d.append((x['h_ph'][x['classification'] > 1]).min()) #h_min_canopy_abs   
    d.append((len(x['h_ph'][x['classification'] == 2]))) #n_ca_photons
    d.append((len(x['h_ph'][x['classification'] == 3]))) #n_toc_photons     
    d.append((x['norm_h'][x['classification'] == 3]).std()) #toc_roughness
    d.append((scipy.stats.skew(x['alongtrack'][x['classification'] == 1]))) #h_te_skew
    d.append((x['h_ph'][x['classification'] == 1]).std()) #h_te_std
    d.append((x['h_ph'][x['classification'] == 1]).max()) #h_te_max
    d.append((x['h_ph'][x['classification'] == 1]).min()) #h_te_min
    d.append((x['h_ph'][x['classification'] == 1]).mean()) #h_te_mean
    d.append((x['h_ph'][x['classification'] == 1]).median()) #h_te_median
    ...
    d.append((x['ph_bcr']).mean()) #avg_bckgrd_counts_reduces
    d.append((x['ph_rate']).mean()) #avg_bckgrd_int_height_reduced
...
def rebin_atl08(atl03, atl08, gt, res, res_field):
    ...
    calc_df = atl03.df.groupby('h_ind').apply(f_mi)
    
    # Compute photon_rate_can
    calc_df['photon_rate_can'] = (calc_df.n_ca_photons + calc_df.n_toc_photons) / calc_df.n_unique_shots
